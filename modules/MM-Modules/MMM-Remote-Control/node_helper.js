...
module.exports = NodeHelper.create(Object.assign({
        ...
        executeQuery: function(query, res) {
            ...
            if (query.action === "RESTART") {
                this.pm2Restart(res, query);
                return true;
            }
            if (query.action === "STOP") {
                this.pm2Stop(res, query);
                return true;
            }
        ...
        pm2Restart: function(res, query) {
            ...
            pm2.connect((err) => {
                ...
                pm2.restart(processName, (err, apps) => {
                    this.sendResponse(res, undefined, { action: query.action, processName: processName });
                    pm2.disconnect();
